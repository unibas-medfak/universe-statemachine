...
public class RegionMachineTests extends AbstractStateMachineTests {
	...
	public void testRegionsInNestedState() throws Exception {
		...
		assertThat(states.stream().map(s -> s.getClass().getName()).sorted().collect(Collectors.toList()))
				.containsExactly("ch.unibas.medizin.universe.statemachine.state.ObjectState",
						"ch.unibas.medizin.universe.statemachine.state.ObjectState",
						"ch.unibas.medizin.universe.statemachine.state.RegionState");
		TestStateMachineListener listener = context.getBean(TestStateMachineListener.class);
		machine.addStateListener(listener);
		machine.start();
		listener.reset(3, 0);
