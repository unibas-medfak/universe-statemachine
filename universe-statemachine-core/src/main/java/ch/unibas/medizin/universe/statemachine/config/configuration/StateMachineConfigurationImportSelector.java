...
public class StateMachineConfigurationImportSelector implements ImportSelector {
	...
	public String[] selectImports(AnnotationMetadata importingClassMetadata) {
		...
		if (attrMap != null && AnnotationAttributes.fromMap(attrMap).getBoolean("contextEvents")) {
			return new String[] { "ch.unibas.medizin.universe.statemachine.event.StateMachineEventPublisherConfiguration",
					"ch.unibas.medizin.universe.statemachine.config.configuration.StateMachineCommonConfiguration" };
		} else {
			return new String[0];
		}
	}
